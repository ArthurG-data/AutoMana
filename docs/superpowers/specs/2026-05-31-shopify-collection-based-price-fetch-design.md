...
Storage path change: downstream `process_to_parquet` globs `f"{market_id}_*/**/*products.json"` — this still matches. No changes needed downstream.

Duplicate products (same card in multiple collections) are deduplicated by the existing `_dedupe_batch` in `process_to_parquet` via `(product_id, date, variation)`.
