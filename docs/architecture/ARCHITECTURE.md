...
- **eBay** (OAuth + listing/search/selling) — the selling side is split along CQS lines into three registered service modules:
  ...
  - **Listing database schema** (see migrations 31, 32, 37, 38, 39):
    ...
    - `app_integration.listing_template` — one-row-per-variant templates for (account × product × condition × finish × language × marketplace). Used to bulk-create or manage listing strategies. Columns: `template_id` (PK), `app_code`, `product_id`, `condition_id`, `finish_id`, `language_id`, `marketplace_id` (default `'15'`), `price_cents`, `quantity`, `created_at`, `updated_at`. Unique constraint on `(app_code, product_id, condition_id, finish_id, language_id, marketplace_id)`.
    - `app_integration.listing_pending_actions` — action queue for delayed listing updates. Columns: `id` (PK), `item_id`, `user_id`, `app_code`, `action_type` (raise/lower/hold/draft), `strategy_kind`, `suggested_price`, `status` (pending/processing/done/failed), `error`, `created_at`, `executed_at`. Indexed on `(created_at WHERE status = 'pending')` and `(item_id)`.
    - `pricing.ebay_scraped_sold` — scraped eBay sold listings. Columns: `scrape_id` (PK), `item_id` (unique), `title`, `source_product_id`, `price_cents`, `currency` (default USD), `condition_id`, `finish_id`, `language_id`, `sold_at`, `scraped_at`, `promoted_to_obs` (flag to track bulk promotion to price observations). Indexed on `(source_product_id WHERE promoted_to_obs = false)` and `(sold_at DESC)`.
  - **Backfill tooling** (see `scripts/backfill_listing_variants.py`): interactive CLI script to populate variant fields (`condition_id`, `finish_id`, `product_id`) on existing `ebay_active_listings` rows created before variant support. Flow: show eBay title → auto-match card name → user picks exact version from all printings → select condition and finish → save to DB. Supports `--seed` (pull live listings first), `--dry-run`, and `--app-code` filtering.
