...
class EbayAuthAPIRepository(ApiRepository):
    ...
    TOKEN_URL = "https://api.sandbox.ebay.com/identity/v1/oauth2/token"  # Base URL for eBay Buy API, use a factory later to create the repos, and have the url in a db
    ...
    async def request_auth_code(self, settings : Dict) -> str:
        ...
        auth_url = f"{self.API_URL}?{urllib.parse.urlencode(params)}"
    ...
    async def  exchange_code_token(self, client_id:str, client_secret:str,code:str, redirect_uri:str)->TokenResponse :
        ...
        response = await self._make_post_request(self.TOKEN_URL, headers=headers, data=data)
    ...
    async def exchange_refresh_token(self, refresh_token: str, app_id: str, secret: str, scope: List[str]) -> dict:
        ...
        access_token = await self._make_post_request(self.TOKEN_URL, headers=headers, data=data)
        return access_token