...
class CollectionRepository(AbstractRepository):
    ...
    async def add(self
        ...
        query = "INSERT INTO collections (collection_name, description, user_id) VALUES ($1, $2, $3)  RETURNING collection_id ;"
    ...
    async def delete_many(self, collection_ids: List[UUID], user_id: UUID):
        query = "UPDATE collections SET is_active = False WHERE collection_id IN ($1) AND user_id = $2;"
        return self.execute_command(query, (collection_ids, user_id))
    
    async def update(self, update_fields, collection_id, user_id):
        counter = 1
        query = "UPDATE collections SET " + ", ".join(f"{k} = ${counter + i}" for i, k in enumerate(update_fields.keys())) + f" WHERE collection_id = ${counter + len(update_fields)} AND user_id =${counter+len(update_fields)+1}"
