...
from backend.modules.auth.errors import (
                                          ...
                                          SessionNotFoundError,  TokenInvalidError)
...
def insert_session(conn : connection, new_session : CreateSession):
    ...
    try:
        ...
        conn.rollback()
        raise SessionCreationError("Failed to insert new session into database")
...
def get_active_session(conn : connection, user_id: UUID):
    try:
        ...
        raise SessionNotFoundError(f"Error retrieving session for user ID {user_id}: {str(e)}")
...
def validate_cookie(cookies: CookiesData, conn: connection):
    ...
    try:
        ...
        raise TokenInvalidError(f"Unexpected error during cookie validation: {str(e)}")
