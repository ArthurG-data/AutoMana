...
def register_app(queryExecutor: QueryExecutor, input:InputEbaySettings, settings : EbaySettings):
    queryExecutor.execute_command(app.register_app_query,(input.app_id, input.redirect_uri, input.response_type, input.secret,input.secret, settings.secret_key))
   
def assign_app(queryExecutor: QueryExecutor, app_id : UUID, ebay_id :str):
    queryExecutor.execute_command(app.assign_user_app_query, (ebay_id, app_id,))
   
def assign_scope(queryExecutor: QueryExecutor, scope : str, app_id : str):
    queryExecutor.execute_command(app.assign_scope_query, (app_id, scope,))
