...
def insert_card_product_reference(batch: shopify_models.BatchProductProces,queryExecutor : QueryExecutor.QueryExecutor):
    query = """
    CALL add_card_product_ref_batch(%s,%s,%s,%s);
    """
    queryExecutor.execute_command(query,  batch.prepare_prodcut_card_batches())
   
def prepare_product_shop_id_query(validated_batch: shopify_models.BatchProductProces) -> Tuple[
    list[str],
    list[str],
    list[int],
    list[datetime],
...
def validate_batch(batch: List[Any]) -> shopify_models.BatchProductProces:
    validated_batch = []
    for idx ,item in enumerate(batch):
        try:
            product : shopify_models.ProductPrice = shopify_models.ProductPrice.model_validate(item)
            validated_batch.append(product)
        except Exception as e:
            raise ValueError(f"Validation error in item {item}: {e}")
    return shopify_models.BatchProductProces(items=validated_batch)
...
def find_condition_variant(product: shopify_models.ProductModel, condition: str) -> Optional[float]:
    """
...
def stream_json_file(path: str, market_id: str, app_id: str, batch_size: int = 1000, product_currency='AUD'):
    ...
    with open(path, 'r', encoding='utf-8') as file:
        ...
        products_model = [shopify_models.ProductModel(**c) for c in items ]
