...
def prices_to_parquet(print_id: int, raw_json: bytes, out_path: Path):
    ...
    if df.empty:
        ...
        df = df.rename(columns={"low":"price_low","avg":"price_avg","high":"price_high","foil":"price_foil", "market":"price_market", "market_foil":"price_market_foil"})
...
async def run_batch(mtg_stock_repository : ApiMtgStockRepository
    ...
    async with mtg_stock_repository as repo:
        input = print_ids if print_ids is not None else list(range(range_start, range_end + 1))
        results = await repo.fetch_card_data_batches(input)
