...
async def add(repository: ShopifyCollectionRepository  , values: shopify_theme.InsertCollection):
    ...
    try:
        await repository.add(values.market_id, values.name)
        value = await repository.get(values.name, values.market_id)
        if not value:
...
async def link_theme(repository: ShopifyCollectionRepository, values: shopify_theme.InsertCollectionTheme):
    """Link a collection to a theme"""
    try:
        value = await repository.link_collection_theme(values.collection_name, values.theme_code)
        if not value:
