...
async def test_update_redirect_uri_returns_true_on_success(repo):
    ...
    result = await repo.update_redirect_uri("user-uuid-123", "my-app", "https://automana.duckdns.org/api/integrations/ebay/auth/callback")
...
async def test_update_redirect_uri_returns_false_when_app_not_found(repo):
    ...
    result = await repo.update_redirect_uri("user-uuid-123", "unknown-app", "https://automana.duckdns.org/api/integrations/ebay/auth/callback")
...
async def test_update_redirect_uri_passes_correct_args(repo):
    ...
    await repo.update_redirect_uri("user-uuid-123", "my-app", url)
    repo.execute_query.assert_called_once()
    call_args = repo.execute_query.call_args
    assert call_args[0][1] == (url, "my-app", "user-uuid-123")
