...
@pytest.mark.asyncio
async def test_add_collection_success(mock_collection_repo, test_collection, test_collection_data_user, test_user):
    mock_collection_repo.add.return_value = test_collection_data_user
    user_id = test_user["unique_id"]
    result = await add(mock_collection_repo, test_collection, user_id)
    result = CollectionInDB.model_validate(result)
    assert isinstance(result, CollectionInDB)
    mock_collection_repo.add.assert_called_once_with(user_id, test_collection)

@pytest.mark.asyncio
async def test_add_collection_failure(mock_collection_repo, test_collection, test_user):
    mock_collection_repo.add.return_value = None
    user_id = test_user["unique_id"]
    with pytest.raises(card_catalog_exceptions.CollectionCreationError):
        await add(mock_collection_repo, test_collection, user_id)
...
@pytest.mark.asyncio
async def test_get_many_all_collections_success(mock_collection_repo, test_collection1_data_user1, test_collection2_data_user1, test_user):
    #test a user if 2 collections exist
    mock_collection_repo.get_many.return_value = [test_collection1_data_user1, test_collection2_data_user1]
    
    user_id = test_user["unique_id"]
    result = await get_many(mock_collection_repo, user_id)
    
    assert isinstance(result, list)
    assert len(result) > 0
    mock_collection_repo.get_many.assert_called_once_with(user_id, None)
...
async def test_get_many_no_collections_found(mock_collection_repo, test_user):
    ...
    user_id = test_user["unique_id"]
    with pytest.raises(card_catalog_exceptions.CollectionNotFoundError):
        await get_many(mock_collection_repo, user_id)
