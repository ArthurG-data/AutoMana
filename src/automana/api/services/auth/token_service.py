from automana.api.schemas.auth.cookie import CookiesData
...
async def validate_session_from_cookie(repository: SessionRepository, cookies: CookiesData):
    session_id = auth.extract_session_id(cookies)
...
async def process_token_refresh(
    ...
    return Token(access_token= token_result, token_type="bearer")
