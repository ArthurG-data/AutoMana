...
class EnhancedSetImportService:
            ...
            async with storage_service.open_stream(filename) as f:
                ...
                for set_json in sets_iter:
                    try:
                        ...
                        if len(batch) >= self.config.batch_size:
                            logger.info("ðŸ“¦ processing batch %s (%s sets)", batch_count + 1, len(batch))
    ...
    async def _process_batch(self, batch: List[NewSet], batch_number: int):
        ...
        while retry_count <= self.config.max_retries:
            try:
                logger.info(f"ðŸ”„ Processing batch {batch_number} with {len(batch)} sets (attempt {retry_count + 1})")
    ...
    def _save_failed_sets(self):
        ...
        try:
            ...
            logger.info(f"ðŸ’¾ Saved {len(self.failed_sets)} failed sets to {failed_file}")
            
        except Exception as e:
            logger.error(f"âŒ Failed to save failed sets: {str(e)}")
    ...
    def _log_processing_summary(self):
        ...
        logger.info(f"ðŸš€ Processing rate: {self.stats.total_sets / max(self.stats.duration_seconds, 1):.1f} sets/second")
        logger.info("=" * 60)
