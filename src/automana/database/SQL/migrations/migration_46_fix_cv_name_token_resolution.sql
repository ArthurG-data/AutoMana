-- migration_46: Fix UndefinedColumnError: column cv.name does not exist
--
-- Root cause: card_catalog.card_version has no `name` column. Card names live on
-- card_catalog.unique_cards_ref (column: card_name), joined via unique_card_id.
-- Both load_staging_prices_batched and resolve_price_rejects contained a token-
-- name resolution branch that referenced cv.name directly — crashing at runtime
-- whenever a token row (collector_number IS NULL, matched by name) was processed.
--
-- Fix: add JOIN card_catalog.unique_cards_ref ucr ON ucr.unique_card_id = cv.unique_card_id
-- and replace all cv.name references with ucr.card_name in both functions.
--
-- Affected functions:
--   pricing.load_staging_prices_batched  — tmp_map_tok block
--   pricing.resolve_price_rejects        — map_tok CTE
--
-- Applied to live DB via CREATE OR REPLACE (idempotent).
-- No schema changes; no grants needed (functions already exist with correct perms).

-- ---------------------------------------------------------------------------
-- 1) load_staging_prices_batched
-- ---------------------------------------------------------------------------
