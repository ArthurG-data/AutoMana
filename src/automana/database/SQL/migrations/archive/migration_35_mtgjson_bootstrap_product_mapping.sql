-- migration_35: Auto-bootstrap product_ref + mtg_card_products for staged cards with no mapping.
--
-- Before this fix, load_price_observation_from_mtgjson_staging_batched silently skipped any
-- card not already in mtg_card_products (which was only populated via MTGStock bulk load).
-- Cards from new/niche sets (e.g. Marvel Spider-Man, Universes Beyond) had pricing in the
-- MTGJson AllPrices archive but could never be promoted to T1.
--
-- This migration:
--   1. Backfills product_ref + mtg_card_products for all currently-staged unmapped cards.
--   2. Replaces the procedure with a version that self-heals on every run.

-- Step 1: backfill existing staging rows that are currently stuck.
WITH unmapped AS (
...
-- Step 2: replace the procedure with the self-healing version.
...
BEGIN
  ...
  WHILE v_start <= v_max LOOP
    ...
    BEGIN
      ...
      -- delete only rows that resolved successfully
      WITH src AS (
        SELECT ps.source_id, ps.code, ps.name, ps.currency_code
        FROM pricing.price_source ps
      ),
