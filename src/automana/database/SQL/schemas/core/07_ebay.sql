...
-- ── eBay Active Listings + Listing Template (migrations 31, 37, 38, 39) ──────────────
...
CREATE TABLE IF NOT EXISTS app_integration.ebay_active_listings (
    ...
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    product_id      UUID         REFERENCES pricing.product_ref(product_id),
    condition_id    SMALLINT     REFERENCES pricing.card_condition(condition_id),
    finish_id       SMALLINT     REFERENCES card_catalog.card_finished(finish_id),
    language_id     SMALLINT     REFERENCES card_catalog.language_ref(language_id),
    marketplace_id  VARCHAR(10),
    template_id     UUID         REFERENCES app_integration.listing_template(template_id),
    title           TEXT,
    match_score     REAL
);
...
-- ── Listing Pending Actions (migration_32) ────────────────────────────────────────────
