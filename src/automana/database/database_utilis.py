...
from automana.api.dependancies.database import get_cursor
from uuid import UUID
from automana.database import errors
...
def exception_handler(exception: Exception):
    ...
    if isinstance(exception, psycopg2.IntegrityError):  # Unique constraint or foreign key issues
        ...
        logger.error("database_error", extra={"action": "database_exception"}, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error.")
    
    else:  # Other unexpected errors
        logger.error("unexpected_error", extra={"action": "database_exception"}, exc_info=True)
...
def delete_rows(connection : connection,
    ...
    query = query_creator_function(is_list, values)
    logger.debug("delete_query_created", extra={"action": "delete_rows"})
    try:
